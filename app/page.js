...
// ---------- Top Nav ----------
const TopNav = ({ navigate, shortlistCount, user, onOpenAuth, intent, onOpenIntent, city, onChangeCity, onSignOut }) => {
  ...
  // Helper to navigate from menu and close it.
  const goAndClose = (route) => { navigate(route); setUserMenuOpen(false); };
        ...
        {user ? (
          <div ref={userMenuRef} style={{ position: 'relative' }}>
            <button
              ...
              className="flex items-center gap-2 pl-1 py-1 rounded-full border border-stone-200 hover:border-[#1A1B3A] transition bg-white"
              style={{ paddingRight: '10px' }}
              aria-expanded={userMenuOpen}
              aria-haspopup="true">
              <div className="rounded-full flex items-center justify-center font-medium" style={{ width: '32px', height: '32px', backgroundColor: '#1A1B3A', color: '#ffffff', fontSize: '12px' }}>{user.name ? user.name[0].toUpperCase() : '?'}</div>
              <span className="text-stone-800 show-md-up" style={{ fontSize: '14px' }}>{user.name ? user.name.split(' ')[0] : 'Account'}</span>
              <ChevronDown size={14} className="text-stone-500" style={{ transform: userMenuOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }}/>
            </button>
            ...
            {userMenuOpen && (
              <div
                ...
                style={{
                  ...
                  borderRadius: '16px',
                  boxShadow: '0 10px 40px -10px rgba(26, 27, 58, 0.18), 0 4px 12px -4px rgba(26, 27, 58, 0.08)',
                  overflow: 'hidden',
                  zIndex: 50,
                }}>
                {/* Header: name + phone */}
                <div className="px-5 py-4" style={{ borderBottom: '1px solid #F1EEE7' }}>
                  <p className="font-display leading-none" style={{ fontSize: '17px', color: '#1A1B3A' }}>{user.name || 'Account'}</p>
                  {user.phone && <p style={{ fontSize: '12px', color: '#71717A', marginTop: '4px' }}>+91 {user.phone}</p>}
                </div>

                {/* Menu items */}
                <div className="py-1">
                  <UserMenuItem icon={User} label="Profile" onClick={() => goAndClose({ name: 'profile' })}/>
                  <UserMenuItem
                    icon={BookmarkCheck}
                    label="Shortlist"
                    badge={shortlistCount > 0 ? shortlistCount : null}
                    onClick={() => goAndClose({ name: 'shortlist' })}/>
                </div>

                {/* Sign out — visually separated */}
                <div style={{ borderTop: '1px solid #F1EEE7' }} className="py-1">
                  <UserMenuItem
                    icon={LogOut}
                    label="Sign out"
                    danger
                    onClick={async () => { setUserMenuOpen(false); await onSignOut(); }}/>
                </div>
              </div>
            )}
          </div>
    ...
    {mobileOpen && (
      <div className="show-mobile border-t border-stone-200" style={{ backgroundColor: '#FAF7F2', flexDirection: 'column' }}>
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6" style={{ padding: '20px 16px', width: '100%' }}>
          ...
          {/* Profile + Sign out (only when signed in) */}
          {user && (
            <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid #E7E5E4' }}>
              <button
                onClick={() => { navigate({ name: 'profile' }); setMobileOpen(false); }}
                className="hover:bg-white transition flex items-center justify-between"
                style={{ fontSize: '15px', color: '#1A1B3A', width: '100%', textAlign: 'left', padding: '14px 12px', borderRadius: '12px', background: 'transparent', border: 'none', cursor: 'pointer' }}>
              ...
              <button
                onClick={async () => { setMobileOpen(false); await onSignOut(); }}
                className="hover:bg-white transition flex items-center justify-between"
                style={{ fontSize: '15px', color: '#B91C1C', width: '100%', textAlign: 'left', padding: '14px 12px', borderRadius: '12px', background: 'transparent', border: 'none', cursor: 'pointer' }}>
...
// Single row in the user dropdown menu.
const UserMenuItem = ({ icon: Icon, label, onClick, badge, danger }) => (
  <button
    onClick={onClick}
    ...
    style={{
      ...
      cursor: 'pointer',
      fontSize: '14px',
      color: danger ? '#B91C1C' : '#1A1B3A',
      textAlign: 'left',
    }}
    onMouseOver={e => { e.currentTarget.style.backgroundColor = danger ? '#FDEEED' : '#FAF7F2'; }}
    ...
    {badge != null && (
      <span className="rounded-full px-2 min-w-[20px] text-center font-medium" style={{ fontSize: '11px', backgroundColor: '#1A1B3A', color: '#ffffff', padding: '2px 7px' }}>{badge}</span>
    )}
  </button>
);

// ---------- Home / Landing — clean centered hero ----------
const HomePage = ({ navigate, listings, toggleShortlist, shortlist, intent, onOpenIntent, user, city, onChangeCity, toggleCompare, compare }) => {
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-20">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', columnGap: '24px', rowGap: '40px' }}>
          {featured.slice(0, 6).map(l => (
            <ListingCard key={l.id} listing={l} navigate={navigate} toggleShortlist={toggleShortlist} shortlisted={shortlist.includes(l.id)} toggleCompare={toggleCompare} comparing={compare.includes(l.id)}/>
          ))}
        </div>
      </section>
...
const ListingCard = ({ listing, navigate, toggleShortlist, shortlisted, toggleCompare, comparing }) => {
  const goTo = () => navigate({ name: 'listing', id: listing.id });
  const toggle = (e) => { e.stopPropagation(); toggleShortlist(listing.id); };
  const toggleCmp = (e) => { e.stopPropagation(); if (toggleCompare) toggleCompare(listing.id); };
    ...
    <article className="group cursor-pointer" onClick={goTo}>
      <div className="relative aspect-[4/3] overflow-hidden rounded-2xl">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2">
          <button onClick={toggle} className="w-10 h-10 rounded-full flex items-center justify-center backdrop-blur transition"
            style={shortlisted ? { backgroundColor: '#FFD66B', color: '#1A1B3A' } : { backgroundColor: 'rgba(255,255,255,0.85)', color: '#44403C' }}
            title={shortlisted ? 'Remove from shortlist' : 'Add to shortlist'}>
            <Heart size={15} fill={shortlisted ? 'currentColor' : 'none'}/>
          </button>
          {toggleCompare && (
            <button onClick={toggleCmp} className="w-10 h-10 rounded-full flex items-center justify-center backdrop-blur transition"
              style={comparing ? { backgroundColor: '#1A1B3A', color: '#ffffff' } : { backgroundColor: 'rgba(255,255,255,0.85)', color: '#44403C' }}
...
// ---------- Search / Results ----------
const SearchPage = ({ navigate, route, listings, toggleShortlist, shortlist, intent, onOpenIntent, user, city, onChangeCity, recentSearches, addRecentSearch, savedSearches, toggleSavedSearch, toggleCompare, compare }) => {
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-10">
        ...
        {view === 'grid' ? (
          loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', columnGap: '24px', rowGap: '40px' }}>
              {filtered.map(l => <ListingCard key={l.id} listing={l} navigate={navigate} toggleShortlist={toggleShortlist} shortlisted={shortlist.includes(l.id)} toggleCompare={toggleCompare} comparing={compare.includes(l.id)}/>)}
            </div>
          )
...
// ---------- Listing Detail ----------
const ListingDetailPage = ({ navigate, route, listings, toggleShortlist, shortlist, user, onContactDealer, intent, onOpenIntent, toggleCompare, compare }) => {
          ...
          <div className="absolute top-4 right-4 flex gap-2">
            ...
            {toggleCompare && (
              <button onClick={() => toggleCompare(listing.id)}
                ...
                style={compare && compare.includes(listing.id) ? { backgroundColor: '#1A1B3A', color: '#ffffff' } : { backgroundColor: 'rgba(255,255,255,0.8)', color: '#3F3F46' }}
                title={compare && compare.includes(listing.id) ? 'Remove from compare' : 'Add to compare'}>
...
// ---------- Shortlist ----------
const ShortlistPage = ({ navigate, listings, toggleShortlist, shortlist, intent, onOpenIntent, user, toggleCompare, compare }) => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-12 min-h-screen">
      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', columnGap: '24px', rowGap: '40px' }}>
          {items.map(l => <ListingCard key={l.id} listing={l} navigate={navigate} toggleShortlist={toggleShortlist} shortlisted={true} toggleCompare={toggleCompare} comparing={compare.includes(l.id)}/>)}
        </div>
      )}
    </div>
  );
...
const AuthModal = ({ open, onClose }) => {
  ...
  // Friendly error mapper — turns ApiError details into something the user can act on.
  const friendlyError = (err) => {
    if (!(err instanceof ApiError)) return 'Something went wrong. Please try again.';
    if (err.status === 422 && err.detail?.detail?.[0]?.msg) return err.detail.detail[0].msg;
    if (err.status === 429) return 'Too many attempts. Please try again in an hour.';
  ...
  const submit = async () => {
    ...
    if (step === 'otp') {
      ...
      try {
        ...
        // If the user doesn't have a name yet (new account), go to name step.
        // Otherwise we're done — close the modal.
        if (!user.name) {
          setStep('name');
        } else {
          handleClose();
  ...
  return (
    <Modal onClose={handleClose}>
      <div className="p-10">
          ...
          {step === 'phone' && (
            <div className="flex items-center border-2 border-stone-200 rounded-xl focus-within:border-[#1A1B3A] transition">
              ...
              <input
                ...
                className="flex-1 px-4 py-4 outline-none text-[15px] bg-transparent"
                autoFocus
                disabled={loading}
              />
            </div>
          ...
          {step === 'otp' && (
            <>
              <input
                ...
                placeholder="– – – –"
                className="w-full px-4 py-5 border-2 border-stone-200 rounded-xl outline-none text-center text-[28px] tracking-[1em] font-display focus:border-[#1A1B3A] transition"
                autoFocus
                disabled={loading}
              />
              <p className="text-[12px] text-stone-500 text-center mt-3">Check your phone for the code</p>
            </>
          )}
          {step === 'name' && (
            <input
              ...
              className="w-full px-4 py-4 border-2 border-stone-200 rounded-xl outline-none text-[15px] focus:border-[#1A1B3A] transition"
              autoFocus
              disabled={loading}
            />
          )}
        ...
        {error && (
          <div className="mt-4 px-4 py-3 rounded-xl" style={{ backgroundColor: '#FDEEED', border: '1px solid #F4C5C0', color: '#7A1F1B', fontSize: '13px' }}>
            {error}
          </div>
        )}
        <button
          onClick={submit}
          disabled={loading}
          className="w-full mt-6 py-4 rounded-xl text-[14px] font-medium transition flex items-center justify-center gap-2"
          style={{
            backgroundColor: loading ? '#52525B' : '#1A1B3A',
            color: '#ffffff',
...
const CompareTray = ({ compare, listings, toggleCompare, clearCompare, navigate }) => {
  if (compare.length === 0) return null;
  const items = compare.map(id => listings.find(l => l.id === id)).filter(Boolean);

  return (
    <div className="fixed bottom-0 left-0 right-0 z-30 pointer-events-none">
      <div className="max-w-[1100px] mx-auto px-4 pb-4 sm:pb-6 pointer-events-auto">
        <div className="rounded-2xl shadow-2xl flex items-stretch overflow-hidden" style={{ backgroundColor: '#1A1B3A', border: '1px solid rgba(255,255,255,0.1)' }}>
          {/* Selection slots */}
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0 px-3 sm:px-5 py-3">
            <div className="flex-shrink-0 hide-md-up">
              <p className="font-display leading-none" style={{ fontSize: '14px', color: '#ffffff' }}>Compare</p>
              <p style={{ fontSize: '10px', color: '#A1A1AA' }}>{items.length} of 3</p>
            </div>
            <p className="font-display leading-none show-md-up" style={{ fontSize: '16px', color: '#ffffff', flexShrink: 0 }}>
              Compare <span style={{ color: '#A1A1AA', fontSize: '13px', marginLeft: '4px' }}>{items.length} of 3</span>
            </p>
            <div className="flex items-center gap-2 overflow-x-auto flex-1" style={{ scrollbarWidth: 'thin' }}>
              {items.map(l => (
                <div key={l.id} className="flex-shrink-0 flex items-center gap-2 pl-1 pr-3 py-1 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }}>
                  <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0">
                    <ListingImage listing={l} className="w-full h-full"/>
                  </div>
                  <span className="whitespace-nowrap truncate max-w-[100px]" style={{ fontSize: '12px', color: '#ffffff' }}>{l.title}</span>
                  <button onClick={() => toggleCompare(l.id)} className="flex-shrink-0" style={{ color: '#A1A1AA', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }} title="Remove">
                    <X size={12}/>
                  </button>
                </div>
              ))}
              ...
              {Array.from({ length: 3 - items.length }).map((_, i) => (
                <div key={`empty-${i}`} className="flex-shrink-0 flex items-center justify-center" style={{ width: '36px', height: '36px', border: '1px dashed rgba(255,255,255,0.2)', borderRadius: '999px' }}>
          ...
          {/* Actions */}
          <div className="flex items-stretch flex-shrink-0" style={{ borderLeft: '1px solid rgba(255,255,255,0.1)' }}>
            <button onClick={clearCompare}
              className="show-md-up px-4"
              style={{ backgroundColor: 'transparent', color: '#A1A1AA', border: 'none', cursor: 'pointer', fontSize: '12px' }}
              title="Clear all">
              Clear
            </button>
            <button onClick={() => navigate({ name: 'compare' })}
              disabled={items.length < 2}
              className="flex items-center gap-2 px-4 sm:px-6"
              style={{
                backgroundColor: items.length >= 2 ? '#FFD66B' : 'rgba(255,255,255,0.05)',
                color: items.length >= 2 ? '#1A1B3A' : '#71717A',
                border: 'none',
        ...
        {items.length < 2 && (
          <p className="text-center mt-2" style={{ fontSize: '11px', color: '#71717A' }}>Add at least one more to compare</p>
        )}
      </div>
    </div>
  );
...
// ---------- Compare Toast ----------
const CompareToast = ({ message }) => {
  if (!message) return null;
  return (
    <div className="fixed top-[100px] left-1/2 -translate-x-1/2 z-50 px-4 py-3 rounded-full shadow-lg flex items-center gap-2" style={{ backgroundColor: '#1A1B3A', color: '#ffffff', fontSize: '13px' }}>
      <Scale size={14} style={{ color: '#FFD66B' }}/>
      {message}
    </div>
  );
};
...
// ---------- Compare Page ----------
const ComparePage = ({ navigate, listings, compare, toggleCompare, clearCompare, toggleShortlist, shortlist, onContactDealer }) => {
  const items = compare.map(id => listings.find(l => l.id === id)).filter(Boolean);

  if (items.length === 0) {
    return (
      <div className="max-w-[800px] mx-auto px-4 sm:px-6 py-24 text-center min-h-[70vh] flex flex-col items-center justify-center">
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6" style={{ backgroundColor: '#FFF4D6' }}>
          <Scale size={26} style={{ color: '#5A3D00' }}/>
        </div>
        <h1 className="font-display leading-tight text-[#1A1B3A]" style={{ fontSize: 'clamp(36px, 6vw, 56px)' }}>Nothing to compare yet</h1>
        <p className="text-stone-600 mt-4 max-w-md" style={{ fontSize: '15px' }}>
          Browse homes and tap the <Scale size={14} className="inline align-middle" strokeWidth={2}/> icon on any listing to add it here. You can compare up to 3 homes side-by-side.
        </p>
        <button onClick={() => navigate({ name: 'search' })}
          className="mt-8 font-medium"
          style={{ fontSize: '14px', padding: '14px 24px', backgroundColor: '#1A1B3A', color: '#ffffff', borderRadius: '999px', border: 'none', cursor: 'pointer' }}>
          Browse homes
        </button>
      </div>
    );
  ...
  const allAmenities = Array.from(new Set(items.flatMap(i => i.amenities)));

  // Determine "best" values for highlighting
  const cheapestId = items.reduce((min, l) => l.price < min.price ? l : min, items[0]).id;
  const largestId = items.reduce((max, l) => l.area > max.area ? l : max, items[0]).id;
  const mostAmenitiesId = items.reduce((max, l) => l.amenities.length > max.amenities.length ? l : max, items[0]).id;

  // CSS styles — defined once, applied via inline style for reliability
  const tableStyle = {
    width: '100%',
  ...
  return (
    <div className="bg-[#FAF7F2] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 pt-8 pb-32">
        <button onClick={() => navigate({ name: 'search' })} className="flex items-center gap-2 text-stone-600 hover:text-[#1A1B3A] mb-5" style={{ fontSize: '13px', background: 'none', border: 'none', cursor: 'pointer' }}>
          <ArrowLeft size={14}/> Back to browse
        </button>

        <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
          <div>
            <p className="tracking-[0.2em] uppercase text-stone-500" style={{ fontSize: '11px' }}>Side by side</p>
            <h1 className="font-display text-[#1A1B3A] mt-1 leading-none" style={{ fontSize: 'clamp(40px, 6vw, 64px)' }}>
              Comparing <em className="italic">{items.length}</em> {items.length === 1 ? 'home' : 'homes'}
            </h1>
          </div>
          <button onClick={clearCompare}
            className="flex items-center gap-2"
            style={{ fontSize: '13px', padding: '10px 18px', backgroundColor: '#ffffff', color: '#1A1B3A', borderRadius: '999px', border: '1px solid #E7E5E4', cursor: 'pointer' }}>
            <RotateCcw size={13}/> Clear all
          </button>
        </div>

        {/* Comparison table — real <table> guarantees row heights align across columns */}
        <div style={{ overflowX: 'auto', paddingBottom: '8px', borderRadius: '24px', border: '1px solid #E7E5E4', backgroundColor: '#ffffff' }}>
          <table style={tableStyle}>
            <tbody>
              {/* Image row */}
              <tr>
                <td style={{ ...labelCellStyle, borderTop: 'none', backgroundColor: '#FAF7F2', height: '200px' }}/>
                {items.map(l => (
                  <td key={l.id} style={{ ...dataCellStyle, borderTop: 'none', padding: 0, height: '200px', position: 'relative' }}>
                    <button onClick={() => navigate({ name: 'listing', id: l.id })} style={{ display: 'block', width: '100%', height: '100%', cursor: 'pointer', border: 'none', padding: 0, background: 'none' }}>
                      <ListingImage listing={l} className="w-full h-full"/>
                    </button>
                    <button onClick={() => toggleCompare(l.id)}
                      className="absolute top-3 right-3"
                      style={{ width: '32px', height: '32px', borderRadius: '999px', backgroundColor: 'rgba(255,255,255,0.95)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      title="Remove">
                      <X size={14}/>
                    </button>
                  </td>
              ...
              {/* Title row */}
              <tr>
                <td style={{ ...labelCellStyle, fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.15em' }}>Home</td>
                {items.map(l => (
                  <td key={l.id} style={dataCellStyle}>
                    <button onClick={() => navigate({ name: 'listing', id: l.id })}
                      style={{ textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                      <p className="font-display leading-tight" style={{ fontSize: '20px', color: '#1A1B3A' }}>{l.title}</p>
                      <p className="text-stone-500" style={{ fontSize: '12px', marginTop: '2px' }}>{l.subtitle}</p>
                    </button>
                  </td>
                ))}
              </tr>
              {/* Asking price */}
              <CompareRow label="Asking price" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => <span className="font-display" style={{ fontSize: '22px', color: '#1A1B3A' }}>₹{l.priceLabel}</span>}
                highlightId={cheapestId} badge="Best price"/>
              <CompareRow label="Per sq.ft" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => <>₹{Math.round(l.price / l.area).toLocaleString('en-IN')}</>}/>
              <CompareRow label="Location" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => <span className="flex items-center gap-1.5"><MapPin size={11} className="text-stone-400"/> {l.locality}</span>}/>
              <CompareRow label="Type" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => l.type}/>
              <CompareRow label="Bedrooms" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => `${l.bhk} BHK`}/>
              <CompareRow label="Bathrooms" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => l.baths}/>
              <CompareRow label="Carpet area" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => `${l.area} sq.ft`}
                highlightId={largestId} badge="Largest"/>
              <CompareRow label="Possession" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => l.possession}/>
              <CompareRow label="Age" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => l.age}/>
              <CompareRow label="Builder" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => l.builder}/>
              <CompareRow label="Verified" items={items} labelStyle={labelCellStyle} cellStyle={dataCellStyle}
                render={l => l.verified
                  ? <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full" style={{ backgroundColor: '#E8F3E8', color: '#2D5A2D', fontSize: '11px', fontWeight: 500 }}><Check size={10}/> Verified</span>
                  : <span className="text-stone-400" style={{ fontSize: '12px' }}>—</span>
                }/>
              {/* Amenities section header */}
              <tr>
                <td style={sectionHeaderStyle}>Amenities</td>
                {items.map(l => (
                  <td key={l.id} style={{ ...sectionHeaderStyle, color: '#5A3D00' }}>
                    {l.id === mostAmenitiesId && items.length > 1 && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full" style={{ backgroundColor: '#FFD66B', color: '#5A3D00', fontSize: '10px', fontWeight: 500, letterSpacing: '0.05em', textTransform: 'uppercase' }}>Most amenities</span>
                    )}
                  </td>
                ))}
              </tr>
              ...
              {allAmenities.map(a => (
                <tr key={a}>
                  <td style={labelCellStyle}>{a}</td>
                  {items.map(l => (
                    <td key={l.id} style={dataCellStyle}>
                      {l.amenities.includes(a)
                        ? <span className="inline-flex items-center justify-center" style={{ width: '24px', height: '24px', borderRadius: '999px', backgroundColor: '#E8F3E8' }}><Check size={12} style={{ color: '#2D5A2D' }} strokeWidth={3}/></span>
                        : <span className="inline-flex items-center justify-center" style={{ width: '24px', height: '24px', borderRadius: '999px', backgroundColor: '#F4F0E8' }}><Minus size={12} className="text-stone-400"/></span>
                      }
                    </td>
                  ))}
                </tr>
              ...
              {/* CTAs row */}
              <tr>
                <td style={{ ...labelCellStyle, backgroundColor: '#FAF7F2' }}/>
                {items.map(l => (
                  <td key={l.id} style={{ ...dataCellStyle, padding: '20px' }}>
                    <button onClick={() => onContactDealer(l, 'callback')}
                      className="w-full font-medium"
                      style={{ fontSize: '13px', padding: '12px', backgroundColor: '#1A1B3A', color: '#ffffff', borderRadius: '10px', border: 'none', cursor: 'pointer' }}>
                      Request callback
                    </button>
                    <button onClick={() => navigate({ name: 'listing', id: l.id })}
                      className="w-full"
                      style={{ marginTop: '8px', fontSize: '12px', padding: '10px', backgroundColor: 'transparent', color: '#1A1B3A', borderRadius: '10px', border: '1px solid #E7E5E4', cursor: 'pointer' }}>
                      View full details
                    </button>
                  </td>
                ))}
        ...
        <p className="text-stone-500 text-center mt-6" style={{ fontSize: '12px' }}>
...
// Helper: one row in the compare table — label + one cell per listing, with optional highlight
const CompareRow = ({ label, items, labelStyle, cellStyle, render, highlightId, badge }) => (
  <tr>
    <td style={labelStyle}>{label}</td>
    {items.map(l => {
      const isHighlight = highlightId === l.id && items.length > 1;
      return (
        <td key={l.id} style={{ ...cellStyle, backgroundColor: isHighlight ? 'rgba(255, 214, 107, 0.12)' : 'transparent' }}>
          <div className="flex items-center justify-between gap-2">
            <span>{render(l)}</span>
            {isHighlight && badge && (
              <span className="inline-flex items-center px-2 py-0.5 rounded-full flex-shrink-0" style={{ backgroundColor: '#FFD66B', color: '#5A3D00', fontSize: '10px', fontWeight: 500, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{badge}</span>
            )}
          </div>
        </td>
      );
...
export default function App() {
  ...
  const { listings, isLoading: listingsLoading, error: listingsError } = useListings();
  const { ids: shortlistIds, toggleItem: toggleShortlistBackend } = useShortlist();
  ...
  const [compare, setCompare] = useState([]);  // array of listing IDs, max 3
  const [compareToast, setCompareToast] = useState(null);  // string or null

  // Shortlist is now backend-backed. We keep `shortlist` as the array of IDs
  // (still used as a Set for `.includes()` checks in many places below).
  // Convert the Set into an array since legacy code uses .includes() / .length.
  const shortlist = Array.from(shortlistIds);

  // Use real listings from backend if available; fall back to mocks for safety.
  // (This fallback should never trigger once the backend is running.)
  const allListings = (listings && listings.length > 0) ? listings : MOCK_LISTINGS;

  // Hydrate intent from user object whenever it changes (sign-in, profile update).
  useEffect(() => {
    if (user) setIntent(user.current_intent || null);
    else setIntent(null);
  }, [user]);

  // Close the auth modal automatically once the user becomes signed in.
  useEffect(() => {
  ...
  const navigate = (r) => {
    setRoute(r);
    if (r.name === 'listing') {
      setViewedIds(prev => new Set(prev).add(r.id));
      logEvent('listing_view', { listing_id: r.id });
    }
    window.scrollTo(0, 0);
  };
  ...
  const addRecentSearch = (search) => {
    ...
    logEvent('search', { query: search.query, city: search.city });
  };
  ...
  // Backend-backed shortlist toggle. Falls through to auth gate if signed out.
  const toggleShortlist = async (id) => {
    if (!user) { setAuthOpen(true); return; }
    const wasIn = shortlistIds.has(id);
    try {
      await toggleShortlistBackend(id);
      logEvent(wasIn ? 'shortlist_remove' : 'shortlist_add', { listing_id: id });
      // After 3rd add, prompt for intent if not set yet.
      if (!wasIn && shortlistIds.size + 1 === 3 && !intent) {
        setTimeout(() => setIntentOpen(true), 400);
      }
    } catch (err) {
  ...
  const toggleCompare = (id) => {
    setCompare(c => {
      if (c.includes(id)) {
        logEvent('compare_remove', { listing_id: id });
        return c.filter(x => x !== id);
      }
      if (c.length >= 3) {
        setCompareToast('You can compare up to 3 homes at a time');
        setTimeout(() => setCompareToast(null), 2800);
        return c;
      }
      logEvent('compare_add', { listing_id: id });
      return [...c, id];
    });
  };
  ...
  // After sign-in, if the user has no intent set, prompt them.
  // Tracked by detecting the user transition from null to set.
  const [previousUserId, setPreviousUserId] = useState(null);
  useEffect(() => {
    if (user && user.id !== previousUserId) {
      if (!user.current_intent) {
        setTimeout(() => setIntentOpen(true), 300);
      }
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] text-[#1A1B3A]" style={{ fontFamily: '"Geist", system-ui, sans-serif' }}>
      ...
      <TopNav navigate={navigate} shortlistCount={shortlist.length} user={user} onOpenAuth={() => setAuthOpen(true)} intent={intent} onOpenIntent={() => setIntentOpen(true)} city={city} onChangeCity={setCity} onSignOut={async () => { await signOut(); navigate({ name: 'home' }); }}/>

      {route.name === 'home' && <HomePage navigate={navigate} listings={allListings} toggleShortlist={toggleShortlist} shortlist={shortlist} intent={intent} onOpenIntent={() => setIntentOpen(true)} user={user} city={city} onChangeCity={setCity} toggleCompare={toggleCompare} compare={compare}/>}
      {route.name === 'search' && <SearchPage navigate={navigate} route={route} listings={allListings} toggleShortlist={toggleShortlist} shortlist={shortlist} intent={intent} onOpenIntent={() => setIntentOpen(true)} user={user} city={city} onChangeCity={setCity} recentSearches={recentSearches} addRecentSearch={addRecentSearch} savedSearches={savedSearches} toggleSavedSearch={toggleSavedSearch} toggleCompare={toggleCompare} compare={compare}/>}
      {route.name === 'listing' && <ListingDetailPage navigate={navigate} route={route} listings={allListings} toggleShortlist={toggleShortlist} shortlist={shortlist} user={user} onContactDealer={handleContactDealer} intent={intent} onOpenIntent={() => setIntentOpen(true)} toggleCompare={toggleCompare} compare={compare}/>}
      {route.name === 'shortlist' && <ShortlistPage navigate={navigate} listings={allListings} toggleShortlist={toggleShortlist} shortlist={shortlist} intent={intent} onOpenIntent={() => setIntentOpen(true)} user={user} toggleCompare={toggleCompare} compare={compare}/>}
      {route.name === 'compare' && <ComparePage navigate={navigate} listings={allListings} compare={compare} toggleCompare={toggleCompare} clearCompare={clearCompare} toggleShortlist={toggleShortlist} shortlist={shortlist} onContactDealer={handleContactDealer}/>}
      {route.name === 'profile' && <ProfilePage user={user} intent={intent} navigate={navigate} onChangeIntent={() => setIntentOpen(true)} onSignOut={async () => { await signOut(); navigate({ name: 'home' }); }} viewedCount={viewedIds.size} shortlist={shortlist} savedSearches={savedSearches} toggleSavedSearch={toggleSavedSearch}/>}
      {route.name === 'about' && <AboutPage navigate={navigate}/>}
      {!['home','search','listing','shortlist','profile','about','compare'].includes(route.name) && <NotFoundPage navigate={navigate}/>}

      <AuthModal open={authOpen} onClose={() => setAuthOpen(false)}/>
      <IntentModal
        open={intentOpen}
        onClose={() => setIntentOpen(false)}
        onSelect={async (newIntent) => {
          setIntent(newIntent);  // optimistic update for instant UI feedback
          if (user) {
            try {
              await apiFetch('/me', { method: 'PATCH', body: { current_intent: newIntent } });
            } catch (err) {
              console.error('Failed to save intent:', err);
              // Roll back the optimistic update on failure.
              setIntent(user.current_intent || null);
            }
          }
        }}
      ...
      {route.name !== 'compare' && <CompareTray compare={compare} listings={allListings} toggleCompare={toggleCompare} clearCompare={clearCompare} navigate={navigate}/>}
      <CompareToast message={compareToast}/>
    </div>
  );
}
