...
const ComparePage = ({ navigate, listings, compare, toggleCompare, clearCompare, toggleShortlist, shortlist, onContactDealer }) => {
    ...
    return (
      <div className="max-w-[800px] mx-auto px-4 sm:px-6 py-24 text-center min-h-[70vh] flex flex-col items-center justify-center">
        ...
        <p className="text-stone-600 mt-4 max-w-md" style={{ fontSize: '15px' }}>
          Browse homes and tap the <Scale size={13} className="inline align-middle"/> icon on any listing to add it here. You can compare up to 3 homes side-by-side.
        </p>
        <button onClick={() => navigate({ name: 'search' })}
  ...
  // Build comparison rows: all unique amenities across all listings, with each row showing checks
  const allAmenities = Array.from(new Set(items.flatMap(i => i.amenities)));

  // Determine "best" values for highlighting (cheapest price, largest area, most amenities)
    ...
    <div className="bg-[#FAF7F2] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 pt-8 pb-32">
        ...
        <div style={{ overflowX: 'auto', paddingBottom: '8px' }}>
          <div style={{
            display: 'grid',
            gridTemplateColumns: `200px repeat(${items.length}, minmax(260px, 1fr))`,
            gap: '0',
            ...
            {/* Column 1 (sticky labels) - intentionally blank header */}
            <div style={{ backgroundColor: '#FAF7F2', borderRight: '1px solid #E7E5E4' }}>
              {/* Image row */}
              <div style={{ height: '200px' }}/>
              {/* Title row */}
              <div className="px-5 py-4" style={{ borderTop: '1px solid #E7E5E4' }}>
                <p className="tracking-[0.15em] uppercase text-stone-500" style={{ fontSize: '11px' }}>Home</p>
              </div>
              {/* Price */}
              <CompareLabel>Asking price</CompareLabel>
              <CompareLabel>Per sq.ft</CompareLabel>
              <CompareLabel>Location</CompareLabel>
              <CompareLabel>Type</CompareLabel>
              <CompareLabel>Bedrooms</CompareLabel>
              <CompareLabel>Bathrooms</CompareLabel>
              <CompareLabel>Carpet area</CompareLabel>
              <CompareLabel>Possession</CompareLabel>
              <CompareLabel>Age</CompareLabel>
              <CompareLabel>Builder</CompareLabel>
              <CompareLabel>Verified</CompareLabel>
              {/* Amenities section header */}
              <div className="px-5 py-4" style={{ borderTop: '1px solid #E7E5E4', backgroundColor: '#F4EFE3' }}>
                <p className="tracking-[0.15em] uppercase font-medium" style={{ fontSize: '11px', color: '#5A3D00' }}>Amenities</p>
              </div>
              {allAmenities.map(a => <CompareLabel key={a} small>{a}</CompareLabel>)}
              {/* CTA row */}
              <div className="px-5 py-4" style={{ borderTop: '1px solid #E7E5E4' }}/>
            </div>
            ...
            {items.map((l) => {
              const ppsf = Math.round(l.price / l.area);
              const isCheapest = l.id === cheapestId && items.length > 1;
              const isLargest = l.id === largestId && items.length > 1;
              const hasMostAmenities = l.id === mostAmenitiesId && items.length > 1;
              return (
                <div key={l.id} style={{ borderRight: '1px solid #E7E5E4' }}>
                  {/* Image */}
                  <div style={{ height: '200px', position: 'relative' }}>
                    <button onClick={() => navigate({ name: 'listing', id: l.id })} style={{ display: 'block', width: '100%', height: '100%', cursor: 'pointer', border: 'none', padding: 0 }}>
                      <ListingImage listing={l} className="w-full h-full"/>
                    </button>
                    <button onClick={() => toggleCompare(l.id)}
                      className="absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center"
                      style={{ backgroundColor: 'rgba(255,255,255,0.9)', border: 'none', cursor: 'pointer' }}
                      title="Remove">
                      <X size={14}/>
                    </button>
                  </div>
                  ...
                  {/* Price */}
                  <CompareCell highlight={isCheapest} badge={isCheapest ? 'Best price' : null}>
                    <span className="font-display" style={{ fontSize: '22px', color: '#1A1B3A' }}>₹{l.priceLabel}</span>
                  </CompareCell>
                  <CompareCell>₹{ppsf.toLocaleString('en-IN')}</CompareCell>
                  <CompareCell>
                    <span className="flex items-center gap-1.5"><MapPin size={11} className="text-stone-400"/> {l.locality}</span>
                  </CompareCell>
                  <CompareCell>{l.type}</CompareCell>
                  <CompareCell>{l.bhk} BHK</CompareCell>
                  <CompareCell>{l.baths}</CompareCell>
                  <CompareCell highlight={isLargest} badge={isLargest ? 'Largest' : null}>
                    {l.area} sq.ft
                  </CompareCell>
                  <CompareCell>{l.possession}</CompareCell>
                  <CompareCell>{l.age}</CompareCell>
                  <CompareCell>{l.builder}</CompareCell>
                  <CompareCell>
                    {l.verified
                  ...
                  <div className="px-5 py-4" style={{ borderTop: '1px solid #E7E5E4', backgroundColor: '#F4EFE3' }}>
                    {hasMostAmenities && <span className="inline-flex items-center px-2 py-0.5 rounded-full" style={{ backgroundColor: '#FFD66B', color: '#5A3D00', fontSize: '10px', fontWeight: 500, letterSpacing: '0.05em', textTransform: 'uppercase' }}>Most amenities</span>}
                  </div>
                  {allAmenities.map(a => (
                    <div key={a} className="px-5 py-3 flex items-center" style={{ borderTop: '1px solid #E7E5E4', minHeight: '46px' }}>
                      {l.amenities.includes(a)
                        ? <span className="inline-flex items-center justify-center w-6 h-6 rounded-full" style={{ backgroundColor: '#E8F3E8' }}><Check size={12} style={{ color: '#2D5A2D' }} strokeWidth={3}/></span>
                        : <span className="inline-flex items-center justify-center w-6 h-6 rounded-full" style={{ backgroundColor: '#F4F0E8' }}><Minus size={12} className="text-stone-400"/></span>
                      }
                    </div>
                  ))}
                  {/* CTA */}
                  <div className="px-5 py-4" style={{ borderTop: '1px solid #E7E5E4' }}>
                    ...
                    <button onClick={() => navigate({ name: 'listing', id: l.id })}
                      ...
                      style={{ fontSize: '12px', padding: '10px', backgroundColor: 'transparent', color: '#1A1B3A', borderRadius: '10px', border: '1px solid #E7E5E4', cursor: 'pointer' }}>
                      View full details
                    </button>
                  </div>
                </div>
...
// Helper components for compare table cells
const CompareLabel = ({ children, small }) => (
  <div className="px-5 py-3 flex items-center" style={{ borderTop: '1px solid #E7E5E4', minHeight: '46px' }}>
    <p style={{ fontSize: small ? '12px' : '12px', color: '#71717A', fontWeight: 500 }}>{children}</p>
  </div>
);

const CompareCell = ({ children, highlight, badge }) => (
  <div className="px-5 py-3 flex items-center justify-between gap-2" style={{ borderTop: '1px solid #E7E5E4', minHeight: '46px', backgroundColor: highlight ? 'rgba(255, 214, 107, 0.12)' : 'transparent', fontSize: '13px', color: '#1A1B3A' }}>
    <span>{children}</span>
    {badge && <span className="inline-flex items-center px-2 py-0.5 rounded-full flex-shrink-0" style={{ backgroundColor: '#FFD66B', color: '#5A3D00', fontSize: '10px', fontWeight: 500, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{badge}</span>}
  </div>
);
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] text-[#1A1B3A]" style={{ fontFamily: '"Geist", system-ui, sans-serif' }}>
      ...
      {route.name === 'home' && <HomePage navigate={navigate} listings={MOCK_LISTINGS} toggleShortlist={toggleShortlist} shortlist={shortlist} intent={intent} onOpenIntent={() => setIntentOpen(true)} user={user} city={city} onChangeCity={setCity} toggleCompare={toggleCompare} compare={compare}/>}
      ...
      {route.name === 'listing' && <ListingDetailPage navigate={navigate} route={route} listings={MOCK_LISTINGS} toggleShortlist={toggleShortlist} shortlist={shortlist} user={user} onContactDealer={handleContactDealer} intent={intent} onOpenIntent={() => setIntentOpen(true)} toggleCompare={toggleCompare} compare={compare}/>}
      {route.name === 'shortlist' && <ShortlistPage navigate={navigate} listings={MOCK_LISTINGS} toggleShortlist={toggleShortlist} shortlist={shortlist} intent={intent} onOpenIntent={() => setIntentOpen(true)} user={user} toggleCompare={toggleCompare} compare={compare}/>}
      {route.name === 'compare' && <ComparePage navigate={navigate} listings={MOCK_LISTINGS} compare={compare} toggleCompare={toggleCompare} clearCompare={clearCompare} toggleShortlist={toggleShortlist} shortlist={shortlist} onContactDealer={handleContactDealer}/>}
      ...
      {route.name !== 'compare' && <CompareTray compare={compare} listings={MOCK_LISTINGS} toggleCompare={toggleCompare} clearCompare={clearCompare} navigate={navigate}/>}
